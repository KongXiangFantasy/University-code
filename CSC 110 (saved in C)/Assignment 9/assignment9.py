...
def create_date(date_num:str)->list[Date]:
    ...
    (2019, 3, 21)
    '''
    given_list = list(date_num.split("-"))
    ...
    return (int(given_list[2])+2000, result, int(given_list[0]))
...
'British TV Shows': ["Monty Python's Almost the Truth"]})
    ...
    for line in list_of_lines:
        line = line.split(',')
        name = line[INPUT_TITLE]
        date = line[INPUT_DATE]
        result_date = create_date(date)
        show_date[name] = result_date
        cast = line[INPUT_CAST].split(':') 
        if cast != ['']:
            actor_name[name] = cast

        category_list = line[INPUT_CATEGORIES].split(':') 
        
        for word in category_list:
            if word not in category:
                name_list = line[INPUT_TITLE].split(',')
                category[word] = name_list
            else: 
                category[word].append(line[INPUT_TITLE])
   
    file_handle.close()
    
    return show_date, actor_name, category
